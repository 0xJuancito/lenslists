...
export default function Page() {
  ...
  useEffect(() => {
    const profileId = pathname
      ?.replace('/users/', '')
      .replace('/favorite-lists', '');

    const loadingLists = fetch(`/api/users/${profileId}/favorite-lists`).then(
      async (res) => {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="flex justify-center">
          {myProfile?.id === user?.id
            ? "You haven't added any list to your favorites. Try adding one!"
            : `${user?.handle} hasn't added any list to their favorites yet.`}
        </div>
      )}
    </div>
  );
